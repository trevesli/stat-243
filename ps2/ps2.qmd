...
Then for each year, find the value(s) corresponding to DVNP & TMAX & March. I use `awk` instead of `grep` since it's more adept at multiple column filtering.
...
It's important to note that the raw `TMAX` values are in tenths of degrees Celsius. It took me too long to figure this out, but it is documented in NOAA's [readme](https://www.ncei.noaa.gov/pub/data/ghcn/daily/readme.txt) for this dataset. So I had to divide all the raw values by 10.
```{python}
#| label: 1c
#| cache: true
...
I was unfamiliar with `try-except`, so I relied on this [Stack Overflow](https://stackoverflow.com/questions/62062226/how-to-work-with-try-and-except-in-python) thread as well as the Python documentation/tutorial on [errors and exceptions](https://docs.python.org/3/tutorial/errors.html) to understand how they work. I also used ChatGPT to assist with building the syntax for my two functions.
