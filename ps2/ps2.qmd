...
echo "The following line should work (no records found):"
...
I was unfamiliar with `try-except`, so I used ChatGPT to assist with the syntax.
```{python}
#| label: 2b
#| cache: true
...
I set up two cases where the code should work:

* One where `get_scholar_citations` returns a bs object
* Another where `get_scholar_info` does not return None when

And another three tests where they shouldn't:

* When an in invalid scholar ID is given
* When a file does not exist or is missing
* If a bad html is passed as the file to be parsed
```{python}
#| label: 2c
#| cache: true
