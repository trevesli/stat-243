...
def test_invalid_domain():
    with pytest.raises(TypeError, match="'domain' must be a tuple."):
        ars(lambda x: np.exp(-x**2), 100, domain="invalid_domain")
    with pytest.raises(ValueError, match="'domain' must contain exactly two elements."):
        ars(lambda x: np.exp(-x**2), 100, domain=(-5,))
    with pytest.raises(ValueError, match="'domain' must contain exactly two elements."):
        ars(lambda x: np.exp(-x**2), 100, domain=())
    with pytest.raises(ValueError, match="'domain' must contain exactly two elements."):
        ars(lambda x: np.exp(-x**2), 100, domain=(-5, 5, 10))
    with pytest.raises(TypeError, match="'domain' must be a tuple."):
        ars(lambda x: np.exp(-x**2), 100, domain=[-5, 5])
    with pytest.raises(ValueError, match="Invalid domain: values must be numeric."):
        ars(lambda x: np.exp(-x**2), 100, domain=(-np.inf, "not_a_number"))
