...
│   ├── sampler_jax.py            # ARS sampling implementation through jax
...
├── setup.py                      # Distribution installation instructions
...
# We can specify the domain and num_init_points
...
plt.plot(bin_centers, true_density, label="True Gaussian", color='red', lw=2)
plt.bar(
    bin_centers, hist, 
    width=bin_edges[1] - bin_edges[0], label="Sampled Histogram", alpha=0.6
    )
plt.xlabel('x', fontsize=14)
plt.ylabel('Density', fontsize=14)
plt.legend()
plt.title('Comparison of Sampled Distribution with True Gaussian', fontsize=18)
plt.show()
```
...
gamma = lambda x: stats.gamma.pdf(x, a=2, scale = 3)

# We can also just plug in the function and the num_samples.
samples = ars.ars(gamma, num_samples=10000)

hist, bin_edges = np.histogram(samples, bins=50, density=True) 
bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])
true_density = stats.gamma.pdf(bin_centers, a=2, scale = 3)

# Plotting
plt.figure(figsize=(8, 6), dpi=300)
plt.plot(bin_centers, true_density, label="True Gamma", color='red', linewidth=2)
plt.bar(bin_centers, hist, \
    width=bin_edges[1] - bin_edges[0], label="Sampled Histogram", alpha=0.6)
plt.xlabel('x', fontsize=14)
plt.ylabel('Density', fontsize=14)
plt.legend()
plt.title('Comparison of Sampled Distribution with True Gamma', fontsize=18)
plt.show()
```
---
